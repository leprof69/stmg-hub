...
export default function AdminImportsPanel() {
  ...
  return (
    <>
      ...
      <div style={{ background: "white", borderRadius: 24, padding: 24, border: `2px solid ${a.COLORS.T}20` }}>
        <h2 style={{ fontFamily: "'Fredoka One', cursive", fontSize: "1.35rem", color: a.COLORS.T, marginBottom: 8 }}>
          {"Importer les missions"}
        </h2>
        <p style={{ color: "#9CA3AF", fontSize: "0.8rem", marginBottom: 16 }}>
          {"Colonnes : id, niveau, difficulte, matiere, theme, chapitre, ordre, titre, contexte, question, mots_cles, correction, xp."}
        </p>
        <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
          <input
            type="file"
            accept=".xlsx"
            onChange={(e) => a.setFichierMissions(e.target.files?.[0] ?? null)}
            style={{
              flex: 1,
              padding: "10px 14px",
              borderRadius: 12,
              border: `2px solid ${a.COLORS.T}30`,
              fontFamily: "'Nunito', sans-serif",
              fontSize: "0.9rem",
            }}
          />
          <AdminBtn
            onClick={() => void a.importerMissions()}
            color={a.COLORS.T}
            disabled={!a.fichierMissions || a.importMissions.loading}
          >
            {a.importMissions.loading ? "\u23f3 Import\u2026" : "Importer"}
          </AdminBtn>
        </div>
        {a.importMissions.message ? (
          <div
            style={{
              marginTop: 12,
              padding: 12,
              borderRadius: 12,
              background: a.importMissions.erreurs > 0 ? a.COLORS.H + "15" : a.COLORS.G + "15",
            }}
          >
            <p
              style={{
                fontFamily: "'Fredoka One', cursive",
                color: a.importMissions.erreurs > 0 ? a.COLORS.H : a.COLORS.G,
                margin: 0,
              }}
            >
              {a.importMissions.message}
        ...
        <div style={{ marginTop: 12 }}>
          <button
            type="button"
            onClick={() => void a.resetMissions()}
            style={{
              background: "none",
              border: `1px solid ${a.COLORS.H}`,
              color: a.COLORS.H,
              fontFamily: "'Fredoka One', cursive",
              fontSize: "0.85rem",
              padding: "8px 20px",
