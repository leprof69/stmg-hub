...
import { getPrimaryDsSdgnTopic, type DsSdgnPremiereTopic } from "./dsSdgnQcmTopics";
...
const CHAPTER_FALLBACK: Record<number, string> = {
...
function questionAlreadyIntegrated(q: string): boolean {
...
function pickSituation(item: SdgnMissionQcm, topic: DsSdgnPremiereTopic): string {
...
/**
 * \u00c9nonc\u00e9 DS unique : mini cas + question, toujours coh\u00e9rents.
 */
export function buildIntegratedDsQuestionText(item: SdgnMissionQcm): string {
  ...
  const situation = pickSituation(item, topic);
